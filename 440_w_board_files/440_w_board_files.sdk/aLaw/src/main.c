...
int parse_wav(const char *filename, WAVFile *wav) {
    ...
    while (1) {
        ...
        if (strncmp(chunkId, "data", 4) == 0) {
            ...
            wav->data_chunk.data = (int16_t*)malloc(chunkSize);
...
void print_wav_info(const WAVFile *wav) {
    ...
    printf("Sample Rate: %lu\n", (unsigned long)wav->format_chunk.sample_rate);
    printf("Byte Rate: %lu\n", (unsigned long)wav->format_chunk.byte_rate);
    printf("Block Align: %u\n", wav->format_chunk.bytes_per_block);
    printf("Bits Per Sample: %u\n", wav->format_chunk.bits_per_sample);
    printf("Data Size: %lu bytes\n", (unsigned long)wav->data_chunk.chunk_size);
...
int write_wav_alaw(const WAVFile *wav, const char *output_filename, const uint8_t *out_buffer) {
    ...
    if (!fp) {
        printf("File system not available, skipping file write.\n");
        return 0;  // Return success to continue program execution
    }
...
uint8_t* alaw(const WAVFile *wav, int total_samples){
    ...
    for (int i = 0; i < total_samples; i++) {
        // Shift 16-bit PCM sample to 12-bit (remove bottom 4 bits)
        int16_t sample = wav->data_chunk.data[i];
        uint16_t index = (sample >> 4) & 0x0FFF;  // ensure 12-bit positive index

        // Lookup A-law from BRAM
        out_buffer[i] = encoded_lut[index];
    }
...
int write_wav_alaw_sd(const WAVFile *wav, const char *output_filename, const uint8_t *out_buffer) {
    ...
    if (!file_buffer) {
        xil_printf("Failed to allocate memory for file buffer\n");
        return 1;
    }
    
    // Clear the buffer
    memset(file_buffer, 0, total_size);
    
    // Copy all parts to the buffer
...
int16_t* pcm(const WAVFile *wav, int total_samples) {
    ...
    printf("Starting PCM decoding...\n");

    for (int i = 0; i < total_samples; i++) {
        uint8_t alaw_sample = ((uint8_t*)wav->data_chunk.data)[i];  // If input is encoded
        out_buffer[i] = decoded_lut[alaw_sample];
    }
    
    printf("PCM decoding finished.\n");
...
int main(void) {
    ...
    printf("decoded_lut:  %p\n", decoded_lut);   // Expect 0x4000xxxx
    printf("speech_data:  %p\n", speech_data);   // Expect 0x0010xxxx

    // Construct fake WAVFile struct using speech_data[]
    WAVFile wav;

    wav.format_chunk.audio_format = 6;
    wav.format_chunk.num_channels = 1;
    wav.format_chunk.sample_rate = 48000;   // Match your software version (48kHz)
    wav.format_chunk.bits_per_sample = 8;
    wav.format_chunk.bytes_per_block = 2;
    wav.format_chunk.byte_rate = 96000;
    wav.data_chunk.data = (int16_t *)speech_data;
    wav.data_chunk.chunk_size = speech_data_len * sizeof(int16_t);  // speech_data_len should be defined in speech.h

    //printf("\nInput (C array) WAV Header:\n");
    //print_wav_info(&wav);
    ...
    // You can choose A-law or PCM path
    ...
    // Try writing to SD card first, fallback to regular file system
    if (write_wav_alaw_sd(&wav, "0:/output_alaw.wav", out_buffer) != 0) {
        xil_printf("SD card write failed, trying regular file system...\n");
        if (write_wav_alaw(&wav, "output_alaw.wav", out_buffer) != 0) {
            fprintf(stderr, "Failed to write A-law WAV\n");
            free(out_buffer);
            return 1;
        }
    } else {
        xil_printf("A-law WAV successfully written to SD card\n");
    }
